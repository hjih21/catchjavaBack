...
public class UserController {
	...
	/*
	 * @PostMapping("/loginMember") public String loginMember(MsgMember mem,
	 * HttpSession session, Model model) { MsgMember member = mapper.login(mem);
	 * logger.info("로그인 된 회원의 정보 >> ", member); session.setAttribute("member",
	 * member); logger.info("로그인 된 회원의 정보: {}", member); if (member != null) {
	 * session.setAttribute("member", member); return "redirect:/main"; } else {
	 * model.addAttribute("errorMsg", "로그인 실패"); return "login"; } }
	 */
	...
	public String loginMember(
	    ...
	    logger.info("로그인 요청 >> id={}, pw={}", user_id, user_pw);

	    MsgMember param = new MsgMember();
	    param.setUser_id(user_id);
	    param.setUser_pw(user_pw);

	    MsgMember member = mapper.login(param);
